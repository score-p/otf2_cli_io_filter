...
public:
    ...
    void verify() const
    {
        REQUIRE(m_region_invocations.at(std::string(TestTrace::RegionName)) == 1);
        REQUIRE(m_locations.count(std::string(TestTrace::LocactionName)) == 1);
        REQUIRE(m_location_groups.count(std::string(TestTrace::LocationGroupName)) == 1);
    }
