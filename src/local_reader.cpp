...
void LocalReader::read_events(OTF2_Reader *reader,
    ...
    OTF2_EvtReaderCallbacks_SetMeasurementOnOffCallback(
        evt_callbacks, event::MeasurementOnOffCb);

    OTF2_EvtReaderCallbacks_SetEnterCallback(evt_callbacks, event::EnterCb);

    OTF2_EvtReaderCallbacks_SetLeaveCallback(evt_callbacks, event::LeaveCb);

    OTF2_EvtReaderCallbacks_SetMpiSendCallback(evt_callbacks, event::MpiSendCb);

    OTF2_EvtReaderCallbacks_SetMpiIsendCallback(evt_callbacks,
                                                event::MpiIsendCb);

    OTF2_EvtReaderCallbacks_SetMpiIsendCompleteCallback(
        evt_callbacks, event::MpiIsendCompleteCb);

    OTF2_EvtReaderCallbacks_SetMpiIrecvRequestCallback(
        evt_callbacks, event::MpiIrecvRequestCb);

    OTF2_EvtReaderCallbacks_SetMpiRecvCallback(evt_callbacks, event::MpiRecvCb);
    ...
    OTF2_EvtReaderCallbacks_SetMpiRequestCancelledCallback(
        evt_callbacks, event::MpiRequestCancelledCb);

    OTF2_EvtReaderCallbacks_SetMpiCollectiveBeginCallback(
        evt_callbacks, event::MpiCollectiveBeginCb);

    OTF2_EvtReaderCallbacks_SetMpiCollectiveEndCallback(
        evt_callbacks, event::MpiCollectiveEndCb);

    OTF2_EvtReaderCallbacks_SetOmpForkCallback(evt_callbacks, event::OmpForkCb);

    OTF2_EvtReaderCallbacks_SetOmpJoinCallback(evt_callbacks, event::OmpJoinCb);
    ...
    OTF2_EvtReaderCallbacks_SetOmpTaskCompleteCallback(
        evt_callbacks, event::OmpTaskCompleteCb);

    OTF2_EvtReaderCallbacks_SetMetricCallback(evt_callbacks, event::MetricCb);

    OTF2_EvtReaderCallbacks_SetParameterStringCallback(
        evt_callbacks, event::ParameterStringCb);
    ...
    OTF2_EvtReaderCallbacks_SetParameterUnsignedIntCallback(
        evt_callbacks, event::ParameterUnsignedIntCb);
    ...
    OTF2_EvtReaderCallbacks_SetRmaCollectiveBeginCallback(
        evt_callbacks, event::RmaCollectiveBeginCb);

    OTF2_EvtReaderCallbacks_SetRmaCollectiveEndCallback(
        evt_callbacks, event::RmaCollectiveEndCb);
    ...
    OTF2_EvtReaderCallbacks_SetThreadTeamBeginCallback(
        evt_callbacks, event::ThreadTeamBeginCb);
    ...
    OTF2_EvtReaderCallbacks_SetCallingContextEnterCallback(
        evt_callbacks, event::CallingContextEnterCb);

    OTF2_EvtReaderCallbacks_SetCallingContextLeaveCallback(
        evt_callbacks, event::CallingContextLeaveCb);

    OTF2_EvtReaderCallbacks_SetCallingContextSampleCallback(
        evt_callbacks, event::CallingContextSampleCb);
    ...
    OTF2_EvtReaderCallbacks_SetIoDestroyHandleCallback(
        evt_callbacks, event::IoDestroyHandleCb);

    OTF2_EvtReaderCallbacks_SetIoDuplicateHandleCallback(
        evt_callbacks, event::IoDuplicateHandleCb);

    OTF2_EvtReaderCallbacks_SetIoSeekCallback(evt_callbacks, event::IoSeekCb);

    OTF2_EvtReaderCallbacks_SetIoChangeStatusFlagsCallback(
        evt_callbacks, event::IoChangeStatusFlagsCb);
