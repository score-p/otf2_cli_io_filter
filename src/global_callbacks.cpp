...
OTF2_CallbackCode GlobalClockPropertiesCb(void *userData,
    ...
    tr->handler().handleGlobalClockProperties(timerResolution, globalOffset,
                                              traceLength);
...
GlobalLocationGroupCb(void *userData, OTF2_LocationGroupRef self,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalLocationGroup(self, name, locationGroupType,
                                            systemTreeParent);
    return OTF2_CALLBACK_SUCCESS;
}
...
OTF2_CallbackCode GlobalLocationCb(void *userData, OTF2_LocationRef self,
    ...
    tr->handler().handleGlobalLocation(self, name, locationType, numberOfEvents,
                                       locationGroup);
    return OTF2_CALLBACK_SUCCESS;
}
...
OTF2_CallbackCode GlobalCallsiteCb(void *userData, OTF2_CallsiteRef self,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalCallsite(self, sourceFile, lineNumber,
...
OTF2_CallbackCode GlobalGroupCb(void *userData, OTF2_GroupRef self,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalGroup(self, name, groupType, paradigm, groupFlags,
...
GlobalMetricMemberCb(void *userData, OTF2_MetricMemberRef self,
                     ...
                     OTF2_StringRef unit) {
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalMetricMember(self, name, description, metricType,
                                           metricMode, valueType, base,
                                           exponent, unit);
    return OTF2_CALLBACK_SUCCESS;
}
...
OTF2_CallbackCode GlobalMetricClassCb(void *userData, OTF2_MetricRef self,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalMetricClass(self, numberOfMetrics, metricMembers,
...
OTF2_CallbackCode GlobalMetricInstanceCb(void *userData, OTF2_MetricRef self,
                                         ...
                                         uint64_t scope) {
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalMetricInstance(self, metricClass, recorder,
                                             metricScope, scope);
    return OTF2_CALLBACK_SUCCESS;
}
...
OTF2_CallbackCode GlobalSystemTreeNodePropertyCb(
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalSystemTreeNodeProperty(systemTreeNode, name, type,
                                                     value);
    return OTF2_CALLBACK_SUCCESS;
}
...
GlobalSystemTreeNodeDomainCb(void *userData,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalSystemTreeNodeDomain(systemTreeNode,
                                                   systemTreeDomain);
    return OTF2_CALLBACK_SUCCESS;
}
...
OTF2_CallbackCode GlobalLocationGroupPropertyCb(
    ...
    OTF2_Type type, OTF2_AttributeValue value) {
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalLocationGroupProperty(locationGroup, name, type,
                                                    value);
    return OTF2_CALLBACK_SUCCESS;
}
...
GlobalCartTopologyCb(void *userData, OTF2_CartTopologyRef self,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalCartTopology(self, name, communicator,
...
OTF2_CallbackCode GlobalCartCoordinateCb(void *userData,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalCartCoordinate(cartTopology, rank,
...
GlobalCallingContextCb(void *userData, OTF2_CallingContextRef self,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalCallingContext(self, region, sourceCodeLocation,
                                             parent);
    return OTF2_CALLBACK_SUCCESS;
}
...
OTF2_CallbackCode GlobalCallingContextPropertyCb(
    ...
    OTF2_Type type, OTF2_AttributeValue value) {
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalCallingContextProperty(callingContext, name, type,
                                                     value);
    return OTF2_CALLBACK_SUCCESS;
}
...
GlobalInterruptGeneratorCb(void *userData, OTF2_InterruptGeneratorRef self,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalInterruptGenerator(
...
OTF2_CallbackCode GlobalIoHandleCb(void *userData, OTF2_IoHandleRef self,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalIoHandle(self, name, file, ioParadigm,
...
GlobalIoPreCreatedHandleStateCb(void *userData, OTF2_IoHandleRef ioHandle,
    ...
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalIoPreCreatedHandleState(ioHandle, mode,
                                                      statusFlags);
    return OTF2_CALLBACK_SUCCESS;
}
...
OTF2_CallbackCode GlobalCallpathParameterCb(void *userData,
                                            ...
                                            OTF2_AttributeValue value) {
    auto tr = static_cast<TraceReader *>(userData);
    tr->handler().handleGlobalCallpathParameter(callpath, parameter, type,
                                                value);
    return OTF2_CALLBACK_SUCCESS;
}
